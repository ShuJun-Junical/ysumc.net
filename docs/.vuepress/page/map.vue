  ...
  <div class="h-dvh">
    ...
    <div class="w-dvw fixed shadow-2xl bottom-0 bg-white/90 backdrop-blur
      ...
      <div>
        ...
        <p v-show="barIsTitle" class="text-sm md:text-base/relaxed" >
        ...
        <div v-show="!barIsTitle">
          <span v-for="i in barTags" :key="i"
            class="inline-block text-white rounded px-3 md:px-4 py-1.5 md:py-2 font-bold text-sm md:text-base/loose" :style="{
      ...
      <div v-show="!barIsTitle" class="text-left md:text-right mt-2 md:mt-0 text-sm md:text-base/relaxed" v-html="barText">
      </div>
    </div>
  </div>
</template>
